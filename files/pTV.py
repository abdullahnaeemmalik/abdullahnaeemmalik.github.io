...
arxiv_graph_bidirected_typeII = add_features_to_graph(
    arxiv_graph_bidirected,partition_indices_and_one_hot_tv)
...
model_kwargs = dict(graph=arxiv_graph_bidirected_typeII, input_layer=getdim_inlayer(arxiv_graph_bidirected_typeII), 
                     hidden_layers=256, output_layer=40, 
                 num_layers=3, dropout=0.5, alpha = 0, beta = 1)
model = GCN(**model_kwargs).to(device)

# Where to save the best model
model_path = 'models'
Path(model_path).mkdir(parents=True, exist_ok=True)
gcn_path = f"{model_path}/gcn_typeII_bidirected.model"

"""Replace name of graph"""
train_args = dict(
    graph=arxiv_graph_bidirected_typeII, labels=labels, split_idx=split_idx, 
    epochs=500, evaluator=evaluator, device=device, 
    save_path=gcn_path, lr=5e-3, es_criteria=50,
)

"""Replace name of graph and features to add"""
print("Training for arxiv_graph_bidirected_typeII with I + A +A^T norm = left using beta = 1 and alpha = 0")
train_losses, val_losses = train(model=model, verbose=True, **train_args)
plot_losses(train_losses, val_losses, log=True)

"""Replace name of graph"""
_ = characterize_performance(model, arxiv_graph_bidirected_typeII, labels, split_idx, evaluator, gcn_path, verbose=True, modelname='typeII_I+A+AT')